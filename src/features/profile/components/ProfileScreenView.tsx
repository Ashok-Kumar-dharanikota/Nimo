...
export function ProfileScreenView() {
  ...
  const { weeklyStreaks, gardenSummary } = useHomeData();
  ...
  const confirmSignOut = async () => {
    ...
    updateProfile({ email: '', name: 'Guest', avatarUri: null, isGuest: true });
    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    router.replace('/auth');
  };

  const streak = useMemo(() => calculateStreak(weeklyStreaks), [weeklyStreaks]);
  const totalMemories = gardenSummary.leavesCount;
  const daysActive = gardenSummary.daysActive;
      ...
      >
        ...
        <Animated.View entering={FadeInDown.delay(200)} className="px-5">
          ...
          <View className="bg-white rounded-[20px] border border-[#efe9e1] overflow-hidden mb-5">
            <ProfileMenuItem icon={Settings} label="Settings" onPress={() => router.push('/settings')} />
            <View className="h-[1px] bg-[#efe9e1] mx-4" />
            <ProfileMenuItem icon={Shield} label="Privacy Policy" onPress={() => router.push('/privacy')} />
            <View className="h-[1px] bg-[#efe9e1] mx-4" />
            <ProfileMenuItem icon={FileText} label="Terms of Service" onPress={() => router.push('/terms')} />
          </View>
        </Animated.View>
